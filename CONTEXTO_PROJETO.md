...
- [x] **Filtro de Dias Produtivos e Remoção de Automação** — Lógica em `main.py` alterada para ignorar dias classificados como `PRODUTIVO`. Limpos 50 registros produtivos anteriores da planilha. O workflow semanal no GitHub Actions foi excluído conforme preferência do usuário (parando a coleta automática semanal). O script agora é executado de forma manual via CLI para períodos específicos.
- [x] **Configuração de Autoria do Git** — Configurada a autoria do Git localmente na pasta para utilizar o nome e e-mail do usuário real ("Antonio Augusto Filho" <aadfilho@gmail.com>).
...
*Última atualização: 15/08/2026 — atualizado por Antigravity. Configurada a autoria do Git para o usuário local e removido o workflow de execução semanal automática do GitHub Actions.*
